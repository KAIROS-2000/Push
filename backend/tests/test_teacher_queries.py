...
class TeacherReadPathTests(unittest.TestCase):
    ...
    def test_class_detail_includes_practice_heatmap(self):
        app = self.create_app()
        classroom_id, lesson_id = self.create_teacher_assignment_fixture(app)

        from app.core.db import db
        from app.core.security import hash_password
        from app.models.learning import Assignment, AssignmentSubmission, encode_assignment_description
        from app.models.user import User, UserRole
        ...
        with app.app_context():
            ...
            db.session.flush()
            practice_a = Assignment(
                classroom_id=classroom_id,
                lesson_id=lesson_id,
                title='Practice A',
                description=encode_assignment_description('Body', 'lesson_practice', 'mixed'),
                difficulty='medium',
                due_date=None,
                xp_reward=0,
            )
            quiz_b = Assignment(
                ...
                lesson_id=lesson_id,
                title='Quiz B',
                description=encode_assignment_description('Quiz body', 'quiz', 'text'),
                difficulty='medium',
                due_date=None,
                xp_reward=0,
            )
            db.session.add_all([practice_a, quiz_b])
            db.session.flush()

            db.session.add_all(
                [
                    AssignmentSubmission(
                        assignment_id=practice_a.id,
                        student_id=s1.id,
                        answer='ok',
                        score=100,
        ...
        with app.test_client() as client:
            ...
            detail = client.get(f'/api/teacher/classes/{classroom_id}')
            self.assertEqual(detail.status_code, 200)
            payload = detail.get_json()
            self.assertIn('practice_heatmap', payload)
            hm = payload['practice_heatmap']
            self.assertEqual(hm['summary']['failed'], 1)
            self.assertEqual(hm['summary']['reviewed'], 2)
            self.assertEqual(hm['summary']['failure_rate'], 50.0)
            self.assertEqual(len(hm['assignments']), 1)
            row = hm['assignments'][0]
            self.assertEqual(row['assignment_id'], practice_id)
            self.assertEqual(row['failed'], 1)
            self.assertEqual(row['reviewed'], 2)
            self.assertEqual(row['failure_rate'], 50.0)
