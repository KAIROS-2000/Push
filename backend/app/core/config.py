...
class Config:
    ...
    # Non-prod default allows both localhost and 127.0.0.1 so dev sessions are not blocked
    # by enforce_origin_for_unsafe_api_requests when the user opens the app via either host.
    CLIENT_URL = _env('CLIENT_URL') or (
        None if IS_PRODUCTION else 'http://localhost:3000,http://127.0.0.1:3000'
    )
    ...
    CLASS_JOIN_RATE_LIMIT_WINDOW_SECONDS = int(_env('CLASS_JOIN_RATE_LIMIT_WINDOW_SECONDS') or '600')
    CLASS_JOIN_RATE_LIMIT_MAX_FAILURES = int(_env('CLASS_JOIN_RATE_LIMIT_MAX_FAILURES') or '12')
    CLASS_JOIN_RATE_LIMIT_BLOCK_SECONDS = int(_env('CLASS_JOIN_RATE_LIMIT_BLOCK_SECONDS') or '600')

    UNISENDER_GO_API_KEY = _env('UNISENDER_GO_API_KEY')
    UNISENDER_GO_API_URL = (
        _env('UNISENDER_GO_API_URL') or 'https://go1.unisender.ru/ru/transactional/api/v1'
    ).rstrip('/')
    UNISENDER_GO_TIMEOUT_MS = int(_env('UNISENDER_GO_TIMEOUT_MS') or '15000')
    EMAIL_PROVIDER = (_env('EMAIL_PROVIDER') or 'unisender_go').strip().lower()
    EMAIL_FROM = _env('EMAIL_FROM') or ''
    EMAIL_FROM_NAME = _env('EMAIL_FROM_NAME') or 'Progyx'
    EMAIL_REPLY_TO = _env('EMAIL_REPLY_TO') or ''
    FRONTEND_PUBLIC_URL = (_env('FRONTEND_PUBLIC_URL') or _env('CLIENT_URL') or '').strip().rstrip('/')
    ...
    RESEND_VERIFICATION_RATE_LIMIT_WINDOW_SECONDS = int(_env('RESEND_VERIFICATION_RATE_LIMIT_WINDOW_SECONDS') or '3600')
    RESEND_VERIFICATION_RATE_LIMIT_MAX_REQUESTS = int(_env('RESEND_VERIFICATION_RATE_LIMIT_MAX_REQUESTS') or '5')
    RESEND_VERIFICATION_RATE_LIMIT_BLOCK_SECONDS = int(_env('RESEND_VERIFICATION_RATE_LIMIT_BLOCK_SECONDS') or '3600')
