...
def submit_task(current_user: User, task_id: int):
    ...
    xp_awarded = 0
    xp_skipped = False
    lesson_age_group = task.lesson.module.age_group if task.lesson else None
    outranks = _user_outranks_lesson(current_user.age_group, lesson_age_group)
...
def submit_quiz(current_user: User, quiz_id: int):
    ...
    lesson_age_group = quiz.lesson.module.age_group if quiz.lesson else None
    outranks = _user_outranks_lesson(current_user.age_group, lesson_age_group)
    if passed:
        progress.status = (
            "pending_review"
