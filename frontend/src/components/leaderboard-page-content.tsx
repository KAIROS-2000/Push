...
export type LeaderboardPageLayout = 'public' | 'admin'
...
function LeaderboardStaffMeta({ row, show }: { row: Row; show: boolean }) {
	if (!show || row.id == null) return null
	const email =
...
export function LeaderboardPageContent({
	...
	const [apiStaffLeaderboard, setApiStaffLeaderboard] = useState(false)
	const { user: sessionUser } = useSessionUser({ auth: 'optional' })

	const showStaffLeaderboard = useMemo(
		() =>
			layout === 'admin' ||
			apiStaffLeaderboard ||
			(sessionUser != null && STAFF_ROLES.has(sessionUser.role)),
		[layout, apiStaffLeaderboard, sessionUser],
	)
	...
	useEffect(() => {
		...
		const loadLeaderboard = async (showSpinner = true) => {
			...
			try {
				...
				setApiStaffLeaderboard(
					Boolean(data.staff_leaderboard) ||
						(data.me?.role != null && STAFF_ROLES.has(data.me.role)),
				)
	...
	const shellClass =
		layout === 'public'
			? 'page-shell mx-auto w-full max-w-[96rem]'
			: 'w-full'
										...
										>
											...
											<div className='mt-3 flex min-h-0 flex-1 items-center gap-3'>
												...
												<div className='min-h-0 min-w-0 flex-1'>
													<h2
														className={`overflow-hidden font-black leading-tight text-slate-900 line-clamp-2 ${
															index === 0
																? 'text-lg sm:text-xl'
																: index === 1
													...
													<LeaderboardStaffMeta
														row={row}
														show={showStaffLeaderboard}
													/>
												</div>
											</div>
								...
								<tbody>
									{rest.map(row => (
										...
										>
											...
											{showStaffLeaderboard ? (
												<td className='px-2 py-3 font-mono text-xs text-slate-600 sm:px-3 sm:py-4'>
													{row.id ?? '—'}
												</td>
											) : null}
											...
											{showStaffLeaderboard ? (
												<td className='max-w-[14rem] break-all px-2 py-3 text-slate-600 sm:px-3 sm:py-4'>
